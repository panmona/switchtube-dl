...
module TubeInfo =
    ...
    let videoInfo token videoId = asyncResult {
        let! response = Api.api RequestType.VideoDetails token videoId |> mapApiError
    ...
    let pathForVideo token videoId = asyncResult {
        let! response = Api.api RequestType.VideoPaths token videoId |> mapApiError
    ...
    let channelInfo token channelId = asyncResult {
        let! response = Api.api RequestType.ChannelDetails token channelId |> mapApiError
    ...
    let channelVideos token channelId = asyncResult {
    ...
    let downloadVideo token path = asyncResult {
        let! response = Api.api RequestType.DownloadVideo token path |> mapApiError
